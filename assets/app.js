...
(function () {
  ...
  function buildSchedule() {
    ...
    plan.items.forEach(function (item, i) {
      ...
      if (i === 0 && pulledTo !== null) {
        notes.push('Day starts at ' + clockLabel(pulledTo) + ' to catch this departure.');
      }
      ...
      if (tour.startTimes.length) {
        ...
        if (next === null) {
          warnings.push('Departs at ' + times.map(clockLabel).join(' or ') +
                        ' — no departure left after the previous stop. Move it earlier in the day.');
        } else {
          if (next > cursor && i > 0) {
            notes.push('Waits ' + durLabel(next - cursor) + ' for the ' + clockLabel(next) + ' departure.');
          }
          start = next;
        }
      }
      ...
      if (weekday !== null && tour.days.indexOf(weekday) === -1) {
        warnings.push('Not offered on ' + DAYS[weekday] + 'days.');
      }
      if (plan.guests > tour.maxGuests) {
        warnings.push('Maximum ' + tour.maxGuests + ' guests.');
      }
      if (plan.guests < tour.minGuests) {
        warnings.push('Minimum ' + tour.minGuests + ' guests.');
      }
  ...
  function renderMenu() {
    ...
    $('tourGrid').innerHTML = shown.map(function (t) {
      var n = counts[t.id] || 0;
      var priceLine = t.priceUnit === 'group'
        ? money(t.price) + ' <small>per group</small>'
        : money(t.price) + ' <small>per guest</small>';
      return '' +
        '<article class="tour-card">' +
          ...
          '<div class="tc-meta">' +
            ...
            '<span>· ' + (t.days.length === 7 ? 'Daily' : t.days.map(function (d) { return DAYS[d]; }).join(' ')) + '</span>' +
          ...
          '<div class="tc-foot">' +
            ...
            (n
              ...
              : '<button class="btn primary small" data-add="' + esc(t.id) + '">Add</button>') +
          '</div>' +
        '</article>';
    }).join('');
  }
  ...
  function renderPlan() {
    ...
    $('planItems').innerHTML = sched.legs.map(function (leg, i) {
      ...
      var per = t.priceUnit === 'group'
        ? money(t.price) + ' per group'
        : money(t.price) + ' × ' + plan.guests + ' guests = ' + money(t.price * plan.guests);
      return '' +
        '<div class="leg">' +
          '<div class="leg-rail">' +
          ...
          '</div>' +
          '<div class="leg-card">' +
            '<h3 class="leg-name">' + esc(t.emoji) + ' ' + esc(t.name) + '</h3>' +
            '<p class="leg-sub">' + durLabel(t.durationMin) + ' · ' + per +
              (t.meetingPoint ? ' · ' + esc(t.meetingPoint) : '') + '</p>' +
            ...
            '<div class="leg-actions">' +
              '<button class="btn small" data-up="' + i + '"' + (i === 0 ? ' disabled' : '') + ' aria-label="Move earlier">↑</button>' +
              '<button class="btn small" data-down="' + i + '"' + (i === sched.legs.length - 1 ? ' disabled' : '') + ' aria-label="Move later">↓</button>' +
              '<span class="spacer"></span>' +
              '<button class="btn small danger" data-drop="' + esc(leg.item.uid) + '">Remove</button>' +
            '</div>' +
          '</div>' +
        '</div>';
    }).join('');
  ...
  function renderAdmin() {
    ...
    $('backupTitle').textContent = live ? 'Backup' : 'Publish & backup';
    ...
    $('adminList').innerHTML = state.tours.length
            ...
            '<div class="admin-row' + (t.active ? '' : ' is-off') + '">' +
              ...
              '<div class="ar-main">' +
                ...
                '<div class="ar-sub">' + esc(t.category) + ' · ' + durLabel(t.durationMin) + ' · ' +
                  money(t.price) + '/' + (t.priceUnit === 'group' ? 'group' : 'guest') +
                  (t.active ? '' : ' · hidden') + '</div>' +
              '</div>' +
              '<div class="ar-tools">' +
      ...
      : '<p class="empty">No tours yet. Tap <em>New tour</em> to build your menu.</p>';
  }
  ...
  function openTourDetail(id) {
    ...
    var body = '' +
      '<div class="detail-hero">' +
        '<div class="tc-emoji">' + esc(t.emoji) + '</div>' +
        '<div><div class="tc-cat">' + esc(t.category) + '</div>' +
        '<div class="tc-name">' + esc(t.name) + '</div></div>' +
      '</div>' +
      '<div class="detail-body"><p>' + esc(t.details || t.blurb) + '</p></div>' +
      (t.includes.length
        ? '<h3 style="font-size:15px;margin:16px 0 6px">Included</h3><ul class="detail-list">' +
          t.includes.map(function (x) { return '<li>' + esc(x) + '</li>'; }).join('') + '</ul>'
        : '') +
      '<h3 style="font-size:15px;margin:18px 0 4px">Details</h3>' +
      '<div class="spec"><span>Duration</span><b>' + durLabel(t.durationMin) + '</b></div>' +
      '<div class="spec"><span>Price</span><b>' + money(t.price) + ' per ' + (t.priceUnit === 'group' ? 'group' : 'guest') + '</b></div>' +
      '<div class="spec"><span>Guests</span><b>' + t.minGuests + '–' + t.maxGuests + '</b></div>' +
      '<div class="spec"><span>Available</span><b>' + (t.days.length === 7 ? 'Daily' : t.days.map(function (d) { return DAYS[d]; }).join(', ')) + '</b></div>' +
      (t.startTimes.length ? '<div class="spec"><span>Departures</span><b>' + t.startTimes.map(function (s) { return clockLabel(toMin(s)); }).join(', ') + '</b></div>' : '') +
      (t.meetingPoint ? '<div class="spec"><span>Meeting point</span><b>' + esc(t.meetingPoint) + '</b></div>' : '');

    openSheet(t.name, body,
      '<button class="btn primary wide" data-add="' + esc(t.id) + '">Add to my tour</button>');
  }
  ...
  function planText() {
    ...
    var lines = [];
    lines.push(state.settings.operator + ' — Build Your Own Tour request');
    lines.push('');
    ...
    sched.legs.forEach(function (l) {
      lines.push('  ' + clockLabel(l.start) + '–' + clockLabel(l.end) + '  ' + l.tour.name +
                 '  (' + durLabel(l.tour.durationMin) + ')');
      if (l.tour.meetingPoint) lines.push('      Meets: ' + l.tour.meetingPoint);
  ...
  function wire() {
    document.body.addEventListener('click', function (e) {
      ...
      switch (e.target.id) {
        ...
        case 'copyPlanBtn': copyText(planText(), 'Itinerary copied'); return;
